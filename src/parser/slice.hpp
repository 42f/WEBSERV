...
struct slice {
   public:
    ...
    slice(const void *data, size_t sz)
        : p(reinterpret_cast<const char *>(data)), size(sz) {}
     ...
     */
    slice take(size_t n) const { return slice(p, std::min(n, size)); }
     ...
     */
    slice from(size_t n) const { return slice(p + n, size - n); }
    /*
     * convert to slice in to string
     */
    std::string to_string() { return std::string(p, size); }

    static std::string to_string_static(slice input) {
        return input.to_string();
    }

    std::string to_string() const { return std::string(p, size); }
    ...
    int icontains(const std::string &other) const {
        ...
        const char *self = p;
        while (*s && *self && len--) {
            if (std::tolower(*s) != std::tolower(*self)) return -1;
            s++;
            self++;
        }
        return 0;
    ...
    bool operator==(const char *str) const {
        ...
        if (s.length() != this->size) return false;
    ...
    bool operator!=(const slice &other) { return !operator==(other); }
