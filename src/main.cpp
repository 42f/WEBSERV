...
void conn_reader(int connfd) {
    ...
    RequestHandler::result_type result =
        handler.update(request.c_str(), request.length());
    std::cout << NC << std::endl;
    ...
    while (respHandler.isReady() == false) {
        std::cout << "Waiting for response to be processed by thread..."
                  << std::endl;
        usleep(30000);
    }
    ...
    if (responseResult.is_err()) {
        Logger::log("Error: Response could not be processed",
                    Logger::toConsole);
...
void simple_listener(void) {
    ...
    if (bind(listenfd, (SA *)&servaddr, sizeof(servaddr)) < 0) exit(2);
    if (listen(listenfd, BACKLOG) < 0) exit(3);

    // infinit loop to listen to the port and respond
    while (true) {
        std::ostringstream message;
        message << "[# " << req_counter++ << "]"
                << " Waiting for connections on port " << SERVER_PORT;
        ...
        connfd = accept(listenfd, (struct sockaddr *)&servaddr_client,
                        (socklen_t *)&len);
